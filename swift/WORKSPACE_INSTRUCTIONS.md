# Workspace Instructions for Swift
...
Your Swift workspaces are stored in:

```
~/.local/share/learn/workspaces/swift/
```

Each lesson has its own directory:

```
~/.local/share/learn/workspaces/swift/stage-<N>/level-<M>/
```
...
├── main.swift    # Your Swift code file
...
- **main.swift** - Blank file where you write your solution
...
- **Right pane**: Your code editor (`main.swift`)
...
```bash
cp -r ~/.local/share/learn/workspaces/swift/ ~/my-learn-backup/
```
...
```bash
cd ~/.local/share/learn/workspaces/swift/
git init
git add .
git commit -m "Backup"
```
...
- **Language specific?** - Check official Swift documentation
...
```bash
cd ~/.local/share/learn/workspaces/swift/stage-X/level-Y
ls                    # List files
cat main.swift   # View your code
vim main.swift   # Edit your code
```

---

Happy learning with Swift!
