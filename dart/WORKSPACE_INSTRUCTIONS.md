# Workspace Instructions for Dart
...
Your Dart workspaces are stored in:

```
~/.local/share/learn/workspaces/dart/
```

Each lesson has its own directory:

```
~/.local/share/learn/workspaces/dart/stage-<N>/level-<M>/
```
...
- **main.dart** - Blank file where you write your solution
...
- **Right pane**: Your code editor (`main.dart`)
...
```bash
cp -r ~/.local/share/learn/workspaces/dart/ ~/my-learn-backup/
```
...
```bash
cd ~/.local/share/learn/workspaces/dart/
git init
git add .
git commit -m "Backup"
```
...
- **Language specific?** - Check official Dart documentation
...
```bash
cd ~/.local/share/learn/workspaces/dart/stage-X/level-Y
ls                    # List files
cat main.dart   # View your code
vim main.dart   # Edit your code
```

---

Happy learning with Dart!
