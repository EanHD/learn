# Workspace Instructions for PowerShell
...
Your PowerShell workspaces are stored in:

```
~/.local/share/learn/workspaces/powershell/
```

Each lesson has its own directory:

```
~/.local/share/learn/workspaces/powershell/stage-<N>/level-<M>/
```
...
├── main.powershell    # Your PowerShell code file
...
- **main.powershell** - Blank file where you write your solution
...
- **Right pane**: Your code editor (`main.powershell`)
...
```bash
cp -r ~/.local/share/learn/workspaces/powershell/ ~/my-learn-backup/
```
...
```bash
cd ~/.local/share/learn/workspaces/powershell/
git init
git add .
git commit -m "Backup"
```
...
- **Language specific?** - Check official PowerShell documentation
...
```bash
cd ~/.local/share/learn/workspaces/powershell/stage-X/level-Y
ls                    # List files
cat main.powershell   # View your code
vim main.powershell   # Edit your code
```

---

Happy learning with PowerShell!
