# Workspace Instructions for SQL
...
Your SQL workspaces are stored in:

```
~/.local/share/learn/workspaces/sql/
```

Each lesson has its own directory:

```
~/.local/share/learn/workspaces/sql/stage-<N>/level-<M>/
```
...
- **main.sql** - Blank file where you write your solution
...
- **Right pane**: Your code editor (`main.sql`)
...
```bash
cp -r ~/.local/share/learn/workspaces/sql/ ~/my-learn-backup/
```
...
```bash
cd ~/.local/share/learn/workspaces/sql/
git init
git add .
git commit -m "Backup"
```
...
- **Language specific?** - Check official SQL documentation
...
```bash
cd ~/.local/share/learn/workspaces/sql/stage-X/level-Y
ls                    # List files
cat main.sql   # View your code
vim main.sql   # Edit your code
```

---

Happy learning with SQL!
