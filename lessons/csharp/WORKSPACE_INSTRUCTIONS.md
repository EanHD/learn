...
Your C# workspaces are stored in:

```
~/.local/share/learn/workspaces/csharp/
```

Each lesson has its own directory:

```
~/.local/share/learn/workspaces/csharp/stage-<N>/level-<M>/
```
...
- **main.csharp** - Blank file where you write your solution
...
- **Right pane**: Your code editor (`main.csharp`)
...
```bash
cp -r ~/.local/share/learn/workspaces/csharp/ ~/my-learn-backup/
```
...
```bash
cd ~/.local/share/learn/workspaces/csharp/
git init
git add .
git commit -m "Backup"
```
...
- **Language specific?** - Check official C# documentation
...
```bash
cd ~/.local/share/learn/workspaces/csharp/stage-X/level-Y
ls                    # List files
cat main.csharp   # View your code
vim main.csharp   # Edit your code
```
