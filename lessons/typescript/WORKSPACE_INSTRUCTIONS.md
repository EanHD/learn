# Workspace Instructions for TypeScript
...
Your TypeScript workspaces are stored in:

```
~/.local/share/learn/workspaces/typescript/
```

Each lesson has its own directory:

```
~/.local/share/learn/workspaces/typescript/stage-<N>/level-<M>/
```
...
├── main.typescript    # Your TypeScript code file
...
- **main.typescript** - Blank file where you write your solution
...
- **Right pane**: Your code editor (`main.typescript`)
...
```bash
cp -r ~/.local/share/learn/workspaces/typescript/ ~/my-learn-backup/
```
...
```bash
cd ~/.local/share/learn/workspaces/typescript/
git init
git add .
git commit -m "Backup"
```
...
- **Language specific?** - Check official TypeScript documentation
...
```bash
cd ~/.local/share/learn/workspaces/typescript/stage-X/level-Y
ls                    # List files
cat main.typescript   # View your code
vim main.typescript   # Edit your code
```
