# Workspace Instructions for Kotlin
...
Your Kotlin workspaces are stored in:

```
~/.local/share/learn/workspaces/kotlin/
```

Each lesson has its own directory:

```
~/.local/share/learn/workspaces/kotlin/stage-<N>/level-<M>/
```
...
├── main.kotlin    # Your Kotlin code file
...
- **main.kotlin** - Blank file where you write your solution
...
- **Right pane**: Your code editor (`main.kotlin`)
...
```bash
cp -r ~/.local/share/learn/workspaces/kotlin/ ~/my-learn-backup/
```
...
```bash
cd ~/.local/share/learn/workspaces/kotlin/
git init
git add .
git commit -m "Backup"
```
...
- **Language specific?** - Check official Kotlin documentation
...
```bash
cd ~/.local/share/learn/workspaces/kotlin/stage-X/level-Y
ls                    # List files
cat main.kotlin   # View your code
vim main.kotlin   # Edit your code
```

---

Happy learning with Kotlin!
